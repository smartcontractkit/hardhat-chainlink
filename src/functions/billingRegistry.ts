...
import {
  FunctionsBillingRegistry__factory,
...
export const getSubscriptionInfo = async (
  ...
  const registry = FunctionsBillingRegistry__factory.connect(
    registryAddress,
    signer
  );
...
export const fundSubscription = async (
  ...
  if (juelsAmount.gt(balance)) {
    throw Error(
      `Insufficent LINK balance. Trying to fund subscription with ${linkAmount} LINK, but only have ${hre.ethers.utils.formatEther(
        balance
      )}.`
    );
  }
