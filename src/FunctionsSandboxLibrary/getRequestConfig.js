...
const getRequestConfig = (unvalidatedConfig) => {
  ...
  if (config.numAllowedQueries) {
    if (
      ...
      throw Error(
        `numAllowedQueries is not correctly specified in config file`
      );
    }
  }
  if (config.secrets || (config.secretsURLs && config.secretsURLs.length > 0)) {
    ...
    if (config.secretsLocation === Location_.Remote) {
      if (!Array.isArray(config.secretsURLs)) {
        throw Error(
          "secretsURLs array is not correctly specified in config file"
        );
      }
      config.secretsURLs.forEach((s) => {
        if (!(0, is_http_url_1.default)(s)) {
          throw Error(
            `invalid HTTP or HTTPs URL ${s} in secretsURLs specified in config file`
          );
        }
      });
    }
  ...
  if (config.args) {
    ...
    for (const arg of config.args) {
      if (typeof arg !== "string") {
        throw Error(
          `an element of the args array is not a string in config file`
        );
      }
    }
  }
  ...
  if (config.expectedReturnType) {
    if (typeof config.expectedReturnType !== "string") {
      throw Error(
        `expectedReturnType is not correctly specified in config file`
      );
    }
