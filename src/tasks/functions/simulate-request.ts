...
export const simulateRequestAction: ActionType<{
  ...
  if (hre.network.config.chainId === undefined) {
    throw new HardhatPluginError(
      "hardhat-chainlink",
      "Current network does not have chainId specified"
    );
  }
  ...
  const clientFactory = await hre.ethers.getContractFactory(
    "FunctionsConsumer"
  );
  ...
  await new Promise(async (resolve: any) => {
    ...
    if (thisRequestConfig.secretsLocation === 1) {
      if (
        !thisRequestConfig.secrets ||
        Object.keys(thisRequestConfig.secrets).length === 0
      ) {
        console.log(
          "Using secrets assigned to the first node as no default secrets were provided"
        );
    ...
    client.on("OCRResponse", async (eventRequestId, presult, err) => {
      ...
      if (presult !== "0x") {
        console.log(
          `Response returned to client contract represented as a hex string: ${presult}\n${getDecodedResultLog(
            thisRequestConfig,
            presult
          )}`
        );
    ...
    registry.on(
      ...
      async (
        ...
        if (requestId === eventRequestId) {
          ...
          console.log(
            `Total estimated cost: ${hre.ethers.utils.formatUnits(
              eventTotalCost,
              18
            )} LINK`
          );
...
const deployMockOracle = async (
  ...
  const oracleFactoryFactory = await hre.ethers.getContractFactoryFromArtifact(
    FunctionsOracleFactoryArtifact
  );
  ...
  const oracle = await hre.ethers.getContractAtFromArtifact(
    FunctionsOracleArtifacts,
  ...
  const registryFactory = await hre.ethers.getContractFactoryFromArtifact(
