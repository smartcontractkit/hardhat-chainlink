export const vrfCoordinatorsRegistry = {
  avalanche: {
    contractAddress: "0xd5d517abe5cf79b7e95ec98db0f0277788aff634",
    chainId: "43114",
    maxGasPrices: [
      {
        keyHash:
          "0x83250c5584ffa93feb6ee082981c5ebe484c865196750b39835ad4f13780435d",
        maxGasPrice: "200000000000",
      },
      {
        keyHash:
          "0x89630569c9567e43c4fe7b1633258df9f2531b62f2352fa721cf3162ee4ecb46",
        maxGasPrice: "500000000000",
      },
      {
        keyHash:
          "0x06eb0e2ea7cca202fc7c8258397a36f33d88568d2522b37aaa3b14ff6ee1b696",
        maxGasPrice: "1000000000000",
      },
    ],
  },
  fuji: {
    contractAddress: "0x2ed832ba664535e5886b75d64c46eb9a228c2610",
    chainId: "43113",
    maxGasPrices: [
      {
        keyHash:
          "0x354d2f95da55398f44b7cff77da56283d9c6c829a4bdf1bbcaf2ad6a4d081f61",
        maxGasPrice: "300000000000",
      },
    ],
  },
  arbitrum: {
    contractAddress: "0x41034678d6c633d8a95c75e1138a360a28ba15d1",
    chainId: "42161",
    maxGasPrices: [
      {
        keyHash:
          "0x08ba8f62ff6c40a58877a106147661db43bc58dabfb814793847a839aa03367f",
        maxGasPrice: "2000000000",
      },
      {
        keyHash:
          "0x72d2b016bb5b62912afea355ebf33b91319f828738b111b723b78696b9847b63",
        maxGasPrice: "30000000000",
      },
      {
        keyHash:
          "0x68d24f9a037a649944964c2a1ebd0b2918f4a243d2a99701cc22b548cf2daff0",
        maxGasPrice: "150000000000",
      },
    ],
  },
  arbitrumGoerli: {
    contractAddress: "0x6d80646beadd07ce68cab36c27c626790bbcf17f",
    chainId: "421613",
    maxGasPrices: [
      {
        keyHash:
          "0x83d1b6e3388bed3d76426974512bb0d270e9542a765cd667242ea26c0cc0b730",
        maxGasPrice: "50000000000",
      },
    ],
  },
  bsc: {
    contractAddress: "0xc587d9053cd1118f25f645f9e08bb98c9712a4ee",
    chainId: "56",
    maxGasPrices: [
      {
        keyHash:
          "0x114f3da0a805b6a67d6e9cd2ec746f7028f1b7376365af575cfea3550dd1aa04",
        maxGasPrice: "200000000000",
      },
      {
        keyHash:
          "0xba6e730de88d94a5510ae6613898bfb0c3de5d16e609c5b7da808747125506f7",
        maxGasPrice: "500000000000",
      },
      {
        keyHash:
          "0x17cd473250a9a479dc7f234c64332ed4bc8af9e8ded7556aa6e66d83da49f470",
        maxGasPrice: "1000000000000",
      },
    ],
  },
  bnbChainTestnet: {
    contractAddress: "0x6a2aad07396b36fe02a22b33cf443582f682c82f",
    chainId: "97",
    maxGasPrices: [
      {
        keyHash:
          "0xd4bb89654db74673a187bd804519e65e3f71a52bc55f11da7601a13dcf505314",
        maxGasPrice: "50000000000",
      },
    ],
  },
  ethereum: {
    contractAddress: "0x271682deb8c4e0901d1a1550ad2e64d568e69909",
    chainId: "1",
    maxGasPrices: [
      {
        keyHash:
          "0x8af398995b04c28e9951adb9721ef74c74f93e6a478f39e7e0777be13527e7ef",
        maxGasPrice: "200000000000",
      },
      {
        keyHash:
          "0xff8dedfbfa60af186cf3c830acbc32c05aae823045ae5ea7da1e45fbfaba4f92",
        maxGasPrice: "500000000000",
      },
      {
        keyHash:
          "0x9fe0eebf5e446e3c998ec9bb19951541aee00bb90ea201ae456421a2ded86805",
        maxGasPrice: "1000000000000",
      },
    ],
  },
  goerli: {
    contractAddress: "0x2ca8e0c643bde4c2e08ab1fa0da3401adad7734d",
    chainId: "5",
    maxGasPrices: [
      {
        keyHash:
          "0x79d3d8832d904592c0bf9818b621522c988bb8b0c05cdc3b15aea1b6e8db0c15",
        maxGasPrice: "150000000000",
      },
    ],
  },
  sepolia: {
    contractAddress: "0x8103b0a8a00be2ddc778e6e7eaa21791cd364625",
    chainId: "11155111",
    maxGasPrices: [
      {
        keyHash:
          "0x474e34a077df58807dbe9c96d3c009b23b3c6d0cce433e59bbf5b34f823bc56c",
        maxGasPrice: "30000000000",
      },
    ],
  },
  fantom: {
    contractAddress: "0xd5d517abe5cf79b7e95ec98db0f0277788aff634",
    chainId: "250",
    maxGasPrices: [
      {
        keyHash:
          "0xb4797e686f9a1548b9a2e8c68988d74788e0c4af5899020fb0c47784af76ddfa",
        maxGasPrice: "4000000000000",
      },
      {
        keyHash:
          "0x5881eea62f9876043df723cf89f0c2bb6f950da25e9dfe66995c24f919c8f8ab",
        maxGasPrice: "10000000000000",
      },
      {
        keyHash:
          "0x64ae04e5dba58bc08ba2d53eb33fe95bf71f5002789692fe78fb3778f16121c9",
        maxGasPrice: "20000000000000",
      },
    ],
  },
  fantomTestnet: {
    contractAddress: "0xbd13f08b8352a3635218ab9418e340c60d6eb418",
    chainId: "4002",
    maxGasPrices: [
      {
        keyHash:
          "0x121a143066e0f2f08b620784af77cccb35c6242460b4a8ee251b4b416abaebd4",
        maxGasPrice: "3000000000000",
      },
    ],
  },
  polygon: {
    contractAddress: "0xae975071be8f8ee67addbc1a82488f1c24858067",
    chainId: "137",
    maxGasPrices: [
      {
        keyHash:
          "0x6e099d640cde6de9d40ac749b4b594126b0169747122711109c9985d47751f93",
        maxGasPrice: "200000000000",
      },
      {
        keyHash:
          "0xcc294a196eeeb44da2888d17c0625cc88d70d9760a69d58d853ba6581a9ab0cd",
        maxGasPrice: "500000000000",
      },
      {
        keyHash:
          "0xd729dc84e21ae57ffb6be0053bf2b0668aa2aaf300a2a7b2ddf7dc0bb6e875a8",
        maxGasPrice: "1000000000000",
      },
    ],
  },
  mumbai: {
    contractAddress: "0x7a1bac17ccc5b313516c5e16fb24f7659aa5ebed",
    chainId: "80001",
    maxGasPrices: [
      {
        keyHash:
          "0x4b09e658ed251bcafeebbc69400383d49f344ace09b9576fe248bb02c003fe9f",
        maxGasPrice: "500000000000",
      },
    ],
  },
};
